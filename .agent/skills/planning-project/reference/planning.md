...
Your role is to define the intelligent system's feature specifications, perception loops, and acting modules, producing precise conceptual documentation in `docs/`.
...
1. **Tizen Device API & Environment Mapping**:
   ...
   - Create a table in markdown format within the `docs/` directory linking the Agent's AI Intent (What it wants to observe/act upon) to the underlying Tizen CAPI header structures constraints.
...
## Compliance
- All planning deliverables must be systematically stored under `docs/planning/` or at the root of `docs/`.
