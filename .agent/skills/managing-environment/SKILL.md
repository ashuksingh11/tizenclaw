...
Since Antigravity natively executes in Windows PowerShell, executing Linux commands directly (e.g., `./deploy.sh`, `gbs build`) will fail fatally or hang the agent loop.
* **Rule**: ALL terminal commands targeting the Tizenclaw daemon MUST be wrapped explicitly under WSL.
* **Pattern**: Always use the format `wsl -e bash -c "..."`. (Example: `wsl -e bash -c "./deploy.sh -a armv7l"`).

## 2. Sequential Constraints for GBS and Cargo (No Background Sub-processes)
When triggering intensive cross-compilation layers generating large object files (e.g., `wsl -e bash -c "gbs build"`), execute synchronously strictly natively blocking.
...
* **Pattern**: Chain them blocking inherently: run `wsl -e bash -c "./deploy.sh -a armv7l -S"` waiting patiently until full GBS cache extraction confirms, prior to spawning the next architecture.
