...
|--------|-------|-----------------|---------|
| Linux filesystem / build / deploy | Bash (via WSL) | `wsl -e bash -c "..."` | `wsl -e bash -c "./deploy.sh"` |
...
```powershell
# ✅ Correct — WSL wrapper
wsl -e bash -c "cd /home/hjhun/samba/github/tizenclaw && ./deploy.sh"
...
Project command?
├── YES → wsl -e bash -c "..."
│   ├── Build/Deploy → wsl -e bash -c "./deploy.sh ..."
