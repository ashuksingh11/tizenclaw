...
impl PluginLlmBackend {
    ...
    fn load_library(&mut self) -> Result<(), String> {
        ...
        if let Some(name_fn) =
            Self::resolve_symbol_static::<PluginGetNameFn>(handle, "TIZENCLAW_LLM_BACKEND_GET_NAME")
        {
    ...
    unsafe fn build_messages_handle(messages: &[LlmMessage]) -> *mut libc::c_void {
        ...
        for msg in messages {
            ...
            if let Ok(c_role) = CString::new(msg.role.as_str()) {
                libtizenclaw_core::llm_types::tizenclaw_llm_message_set_role(
                    msg_h,
                    c_role.as_ptr(),
                );
            }
            if let Ok(c_text) = CString::new(msg.text.as_str()) {
                libtizenclaw_core::llm_types::tizenclaw_llm_message_set_text(
                    msg_h,
                    c_text.as_ptr(),
                );
            }
            ...
            for tc in &msg.tool_calls {
                ...
                if let Ok(c_id) = CString::new(tc.id.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_tool_call_set_id(
                        tc_h,
                        c_id.as_ptr(),
                    );
                }
                if let Ok(c_name) = CString::new(tc.name.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_tool_call_set_name(
                        tc_h,
                        c_name.as_ptr(),
                    );
                }
                ...
                if let Ok(c_args) = CString::new(args_str.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_tool_call_set_args_json(
                        tc_h,
                        c_args.as_ptr(),
                    );
                }
            ...
            if !msg.tool_name.is_empty() {
                if let Ok(c_tn) = CString::new(msg.tool_name.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_message_set_tool_name(
                        msg_h,
                        c_tn.as_ptr(),
                    );
                }
            ...
            if !msg.tool_call_id.is_empty() {
                if let Ok(c_tid) = CString::new(msg.tool_call_id.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_message_set_tool_call_id(
                        msg_h,
                        c_tid.as_ptr(),
                    );
                }
            ...
            if !msg.tool_result.is_null() {
                ...
                if let Ok(c_tr) = CString::new(tr_str.as_str()) {
                    libtizenclaw_core::llm_types::tizenclaw_llm_message_set_tool_result_json(
                        msg_h,
                        c_tr.as_ptr(),
                    );
                }
    ...
    unsafe fn build_tools_handle(tools: &[LlmToolDecl]) -> *mut libc::c_void {
        ...
        for tool in tools {
            ...
            if let Ok(c_desc) = CString::new(tool.description.as_str()) {
                libtizenclaw_core::llm_types::tizenclaw_llm_tool_set_description(
                    tool_h,
                    c_desc.as_ptr(),
                );
            }
            ...
            if let Ok(c_params) = CString::new(params_str.as_str()) {
                libtizenclaw_core::llm_types::tizenclaw_llm_tool_set_parameters_json(
                    tool_h,
                    c_params.as_ptr(),
                );
            }
    ...
    unsafe fn extract_response(response_h: *mut libc::c_void) -> LlmResponse {
        ...
        libtizenclaw_core::llm_types::tizenclaw_llm_response_get_error_message(
            response_h,
            &mut err_ptr,
        );
        if !err_ptr.is_null() {
        ...
        libtizenclaw_core::llm_types::tizenclaw_llm_response_get_prompt_tokens(
            response_h,
            &mut prompt_tokens,
        );
        libtizenclaw_core::llm_types::tizenclaw_llm_response_get_completion_tokens(
            response_h,
            &mut completion_tokens,
        );
        libtizenclaw_core::llm_types::tizenclaw_llm_response_get_total_tokens(
            response_h,
            &mut total_tokens,
        );
        libtizenclaw_core::llm_types::tizenclaw_llm_response_get_http_status(
            response_h,
            &mut http_status,
        );
        ...
        unsafe extern "C" fn collect_tool_call(
            ...
            libtizenclaw_core::llm_types::tizenclaw_llm_tool_call_get_args_json(
                tc_h,
                &mut args_ptr,
            );
            ...
            let id = if id_ptr.is_null() {
                ...
                let s = CStr::from_ptr(id_ptr).to_string_lossy().into_owned();
                libc::free(id_ptr as *mut _);
                s
            };
...
impl LlmBackend for PluginLlmBackend {
    fn initialize(&mut self, config: &Value) -> bool {
        ...
        if let Some(init_fn) =
            self.resolve_symbol::<PluginInitFn>(handle, "TIZENCLAW_LLM_BACKEND_INITIALIZE")
        {
            ...
            if !result {
                log::error!(
                    "Plugin '{}' TIZENCLAW_LLM_BACKEND_INITIALIZE returned false",
                    self.name
                );
            }
            result
            ...
            log::warn!(
                "Plugin '{}' has no TIZENCLAW_LLM_BACKEND_INITIALIZE symbol, skipping init",
                self.name
            );
            true
        }
    ...
    async fn chat(
        ...
        let chat_fn =
            match self.resolve_symbol::<PluginChatFn>(handle, "TIZENCLAW_LLM_BACKEND_CHAT") {
                Some(f) => f,
                None => {
                    return LlmResponse {
                        success: false,
    ...
    fn shutdown(&mut self) {
        if let Some(handle) = self.lib_handle.take() {
            ...
            if let Some(shutdown_fn) = Self::resolve_symbol_static::<PluginShutdownFn>(
                ...
                unsafe { shutdown_fn() };
                log::info!(
                    "Called TIZENCLAW_LLM_BACKEND_SHUTDOWN for plugin '{}'",
                    self.name
                );
            }
