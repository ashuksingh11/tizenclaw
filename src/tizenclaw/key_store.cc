...
std::string KeyStore::DeriveKey(
    const std::string& key_path) {
  std::string path = key_path.empty()
      ? kDefaultKeyPath : key_path;
  ...
  if (machine_id.empty()) {
    LOG(WARNING) << "No machine-id at " << path
                 << ", using fallback";
...
std::string KeyStore::Base64Encode(
...
std::string KeyStore::Base64Decode(
    const std::string& encoded) {
  gsize out_len = 0;
  guchar* decoded = g_base64_decode(
      encoded.c_str(), &out_len);
  if (!decoded) return "";
  std::string result(
    ...
    for (auto& [name, backend] :
      ...
      if (backend.contains("api_key")) {
        ...
        if (!api_key.empty() &&
            !IsEncrypted(api_key)) {
          backend["api_key"] =
              Encrypt(api_key, key_path);
          changed = true;
          LOG(INFO)
              << "Encrypted api_key for: "
              << name;
        }
      }
    }
