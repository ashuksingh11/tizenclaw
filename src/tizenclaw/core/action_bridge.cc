...
constexpr const char* kWorkerName =
    "action_worker";
...
bool ActionBridge::Start() {
  ...
  LOG(INFO)
      << "ActionBridge: Starting worker thread";

  // Create tizen-core task (sub-thread)
  int ret = tizen_core_task_create(
      kWorkerName, true, &task_);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to create task: "
        << ret;
    return false;
  }

  ret = tizen_core_task_get_tizen_core(
      task_, &core_);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to get core: "
        << ret;
  ...
  ret = tizen_core_channel_make_pair(
      &req_sender_, &req_receiver_);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to create "
        << "request channel: " << ret;
    tizen_core_task_destroy(task_);
    task_ = nullptr;
    return false;
  ...
  ret = tizen_core_add_channel(
      core_, req_receiver_,
      OnRequestReceived, this, &req_source_);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to add channel: "
        << ret;
    Stop();
    return false;
  }
  ...
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to run task: "
        << ret;
    Stop();
    return false;
  }
...
std::string ActionBridge::ExecuteAction(
  ...
  nlohmann::json payload = {
      ...
      {"arguments", arguments}};
  return SendRequest(
      ActionCommand::kExecute, payload.dump());
}
...
void ActionBridge::SyncActionSchemas() {
  ...
  auto result = SendRequest(
      ActionCommand::kSync, "{}");
  LOG(INFO)
      << "ActionBridge: Sync result: " << result;
}

void ActionBridge::SetChangeCallback(
    ActionChangeCallback cb) {
...
void ActionBridge::EnsureActionsDir() {
  ...
  if (ec) {
    LOG(WARNING)
        << "ActionBridge: Failed to create "
        << kActionsDir << ": " << ec.message();
  }
}

void ActionBridge::WriteActionMd(
    const std::string& name,
    const nlohmann::json& schema) {
  EnsureActionsDir();

  std::string path =
      std::string(kActionsDir) + "/" + name
      + ".md";
  ...
  if (schema.contains("inputSchema") &&
    ...
    for (auto it = props.begin();
      ...
      std::string ptype =
          it.value().value("type", "string");
      std::string pdesc =
          it.value().value("description", "");
  ...
  LOG(INFO)
      << "ActionBridge: Wrote " << path;
}

void ActionBridge::RemoveActionMd(
    const std::string& name) {
  std::string path =
      std::string(kActionsDir) + "/" + name
      + ".md";
  ...
  if (ec) {
    LOG(WARNING)
        << "ActionBridge: Failed to remove "
        << path << ": " << ec.message();
  } else {
    LOG(INFO)
        << "ActionBridge: Removed " << path;
  }
}

std::vector<nlohmann::json>
ActionBridge::LoadCachedActions() const {
  namespace fs = std::filesystem;
  ...
  for (const auto& entry :
       fs::directory_iterator(kActionsDir, ec)) {
...
std::string ActionBridge::SendRequest(
    ActionCommand cmd,
  ...
  int ret = tizen_core_channel_object_create(
      &obj);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    std::lock_guard<std::mutex> lock(
        pending_mutex_);
  ...
  ret = tizen_core_channel_sender_send(
      req_sender_, obj);
  tizen_core_channel_object_destroy(obj);

  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to send "
        << "request: " << ret;
    std::lock_guard<std::mutex> lock(
        pending_mutex_);
    pending_.erase(id);
...
void ActionBridge::OnRequestReceived(
    tizen_core_channel_object_h object,
    void* user_data) {
  auto* self =
      static_cast<ActionBridge*>(user_data);
  ...
  try {
    ...
    auto cmd = static_cast<ActionCommand>(
        req.value("cmd", 0));
    std::string payload =
        req.value("payload", "{}");
...
void ActionBridge::HandleRequest(
  ...
  if (!client_) {
    ...
    ret = action_client_add_event_handler(
        client_, OnActionEvent, this,
        &event_handler_);
    if (ret != ACTION_ERROR_NONE) {
      LOG(WARNING)
          << "ActionBridge: Failed to add "
          << "event handler: " << ret;
      event_handler_ = nullptr;
    } else {
      LOG(INFO)
          << "ActionBridge: Event handler "
          << "registered";
    }

    LOG(INFO)
        << "ActionBridge: Action client ready";
  }
  ...
  if (cmd == ActionCommand::kSync) {
    ...
    try {
      ...
      LOG(INFO)
          << "ActionBridge: Executing action '"
          << action_name << "' exec_id="
          << exec_id;

      int ret = action_client_execute(
          client_, model_str.c_str(),
          OnActionResult, this);
      if (ret != ACTION_ERROR_NONE) {
        LOG(ERROR)
            << "ActionBridge: Execute failed: "
            << ret;
        {
          std::lock_guard<std::mutex> lock(
              exec_map_mutex_);
...
void ActionBridge::DoSync(int id) {
  ...
  action_client_foreach_action(
      ...
      [](const action_h action,
         void* user_data) -> bool {
        auto* sd =
            static_cast<SyncData*>(user_data);
  ...
  LOG(INFO)
      << "ActionBridge: Synced "
      << sync_data.names.size() << " actions";

  SendResponse(id,
      "{\"synced\":" +
      std::to_string(sync_data.names.size()) +
      "}");
}

// ---- Action event handling ----

void ActionBridge::OnActionEvent(
  ...
  LOG(INFO)
      << "ActionBridge: Action event: "
      << type_str << " for '" << name << "'";

  if (event_type ==
      ACTION_EVENT_TYPE_UNINSTALL) {
    RemoveActionMd(name);
  } else {
    // INSTALL or UPDATE: fetch schema and write
    action_h action_handle = nullptr;
    int ret = action_client_get_action(
        self->client_, name.c_str(),
        &action_handle);
    if (ret == ACTION_ERROR_NONE &&
        action_handle) {
      char* schema_str = nullptr;
      action_get_schema(
          action_handle, &schema_str);
      if (schema_str) {
        try {
          auto schema =
...
void ActionBridge::OnActionResult(
    int execution_id,
  ...
  LOG(INFO)
      << "ActionBridge: Action result for "
      << "exec_id=" << execution_id;

  int req_id = -1;
  {
    std::lock_guard<std::mutex> lock(
        self->exec_map_mutex_);
    auto it =
        self->exec_to_req_.find(execution_id);
  ...
  if (req_id < 0) {
    LOG(WARNING)
        << "ActionBridge: No pending request "
...
void ActionBridge::SendResponse(
    int id, const std::string& result) {
  // Send via response channel
  tizen_core_channel_object_h obj = nullptr;
  int ret =
      tizen_core_channel_object_create(&obj);
  if (ret != TIZEN_CORE_ERROR_NONE) {
    LOG(ERROR)
        << "ActionBridge: Failed to create "
