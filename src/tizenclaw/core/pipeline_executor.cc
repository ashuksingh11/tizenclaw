...
// Skill Pipeline Engine implementation
...
std::string PipelineExecutor::GetPipelinesDir()
    const {
...
std::string PipelineExecutor::StepTypeToString(
...
PipelineStepType
PipelineExecutor::StringToStepType(
    const std::string& s) {
  if (s == "prompt") return PipelineStepType::kPrompt;
  if (s == "condition")
    return PipelineStepType::kCondition;
...
void PipelineExecutor::LoadPipelines() {
  ...
  LOG(INFO) << "Loaded " << pipelines_.size()
            << " pipelines";
}

Pipeline PipelineExecutor::LoadPipelineFile(
  ...
  if (j.contains("steps") &&
    ...
    for (auto& sj : j["steps"]) {
      ...
      step.else_step =
          sj.value("else_step", "");
      step.skip_on_failure =
          sj.value("skip_on_failure", false);
      step.max_retries =
          sj.value("max_retries", 0);
      step.output_var =
          sj.value("output_var", "");
      if (sj.contains("args")) {
        step.args = sj["args"];
...
void PipelineExecutor::SavePipeline(
    const Pipeline& p) const {
  std::string dir = GetPipelinesDir();
  mkdir(dir.c_str(), 0755);
  ...
  for (auto& step : p.steps) {
    ...
    if (!step.tool_name.empty())
      sj["tool_name"] = step.tool_name;
    if (!step.args.is_null() &&
        !step.args.empty())
      sj["args"] = step.args;
    if (!step.prompt.empty())
      sj["prompt"] = step.prompt;
    if (!step.condition.empty())
      sj["condition"] = step.condition;
    if (!step.then_step.empty())
      sj["then_step"] = step.then_step;
    if (!step.else_step.empty())
      sj["else_step"] = step.else_step;
    if (step.skip_on_failure)
      sj["skip_on_failure"] = true;
    if (step.max_retries > 0)
      sj["max_retries"] = step.max_retries;
    if (!step.output_var.empty())
      sj["output_var"] = step.output_var;
    steps.push_back(sj);
  }
  j["steps"] = steps;
...
std::string PipelineExecutor::CreatePipeline(
  ...
  SavePipeline(p);
  LOG(INFO) << "Created pipeline: " << p.id
            << " (" << p.name << ")";

  return p.id;
}

nlohmann::json PipelineExecutor::ListPipelines()
    const {
  std::lock_guard<std::mutex> lock(
      pipelines_mutex_);
...
bool PipelineExecutor::DeletePipeline(
    const std::string& id) {
  std::lock_guard<std::mutex> lock(
      pipelines_mutex_);
...
const Pipeline* PipelineExecutor::GetPipeline(
...
std::string PipelineExecutor::Interpolate(
  ...
  while (std::regex_search(
    ...
    if (dot != std::string::npos) {
      std::string base =
          var_name.substr(0, dot);
      std::string field =
          var_name.substr(dot + 1);
...
bool PipelineExecutor::EvalCondition(
  ...
  if (pos != std::string::npos) {
    std::string left =
        resolved.substr(0, pos);
    std::string right =
        resolved.substr(pos + 4);
    return left == right;
  }
...
std::string PipelineExecutor::ExecuteStep(
  ...
  if (step.type == PipelineStepType::kTool) {
    ...
    if (step.tool_name == "execute_code") {
      std::string code =
          resolved_args.value("code", "");
      nlohmann::json call_args = {
          {"code", code}};
      result = agent_->ExecuteSessionOp(
          "create_session",  // dummy — we call
    ...
    LOG(INFO)
        << "Pipeline step [" << step.id
        << "]: prompt (" << resolved_prompt.size()
        << " chars)";

    result = agent_->ProcessPrompt(
        "pipeline_" + session_id,
        resolved_prompt);
  }
...
PipelineRunResult PipelineExecutor::RunPipeline(
  ...
  while (step_idx < pipeline.steps.size()) {
    ...
    if (step.type ==
        PipelineStepType::kCondition) {
      bool cond_result =
          EvalCondition(step.condition, vars);
