...
bool ToolPolicy::LoadConfig(
    const std::string& config_path) {
  std::ifstream f(config_path);
  if (!f.is_open()) {
    LOG(INFO) << "No tool policy config at "
              << config_path
              << ", using defaults";
  ...
  try {
    ...
    if (j.contains("max_repeat_count")) {
      config_.max_repeat_count =
          j["max_repeat_count"].get<int>();
    }

    if (j.contains("blocked_skills")) {
      for (auto& s : j["blocked_skills"]) {
        config_.blocked_skills.insert(
            s.get<std::string>());
      }
    }
    ...
    if (j.contains("risk_overrides")) {
      for (auto& [k, v] :
           j["risk_overrides"].items()) {
        config_.risk_levels[k] =
            ParseRiskLevel(
                v.get<std::string>());
      }
    }
    ...
    LOG(INFO) << "Tool policy loaded: "
              ...
              << config_.max_repeat_count
              << ", blocked="
              << config_.blocked_skills.size()
              << ", overrides="
              << config_.risk_levels.size();
    return true;
  } catch (const std::exception& e) {
    LOG(ERROR)
        << "Failed to parse tool policy: "
        << e.what();
    return false;
  }
}

void ToolPolicy::LoadManifestRiskLevel(
  ...
  if (manifest.contains("risk_level")) {
    ...
    if (config_.risk_levels.find(skill_name)
        == config_.risk_levels.end()) {
      config_.risk_levels[skill_name] =
          ParseRiskLevel(level_str);
    }
  }
}

std::string ToolPolicy::CheckPolicy(
  ...
  if (config_.blocked_skills.count(
          skill_name) > 0) {
...
RiskLevel ToolPolicy::GetRiskLevel(
  ...
  auto it = config_.risk_levels.find(
      skill_name);
...
std::string ToolPolicy::RiskLevelToString(
    RiskLevel level) {
  switch (level) {
    case RiskLevel::kLow: return "low";
    case RiskLevel::kHigh: return "high";
    default: return "normal";
  }
}

std::string ToolPolicy::HashCall(
    const std::string& name,
    const nlohmann::json& args) const {
  // Simple hash: skill_name + sorted JSON args
  std::string input = name + ":";
  if (!args.is_null()) {
    input += args.dump(-1, ' ', false,
  ...
  std::size_t h =
      std::hash<std::string>{}(input);
  std::ostringstream oss;
  oss << std::hex << h;
  return oss.str();
}

RiskLevel ToolPolicy::ParseRiskLevel(
