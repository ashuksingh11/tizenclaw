...
static LIB_DLOG: LazyLock<Option<Library>> = LazyLock::new(|| unsafe { Library::new("libdlog.so.0").or_else(|_| Library::new("libdlog.so")).ok() });
...
pub mod tizen_core {
    ...
    pub unsafe fn tizen_core_shutdown() -> c_int {
        dlsym_call!(LIB_TIZEN_CORE, b"tizen_core_shutdown\0", unsafe extern "C" fn() -> c_int, 0)
    }

    pub unsafe fn tizen_core_task_create(name: *const c_char, use_thread: c_int, task: *mut tizen_core_task_h) -> c_int {
        dlsym_call!(LIB_TIZEN_CORE, b"tizen_core_task_create\0", unsafe extern "C" fn(*const c_char, c_int, *mut tizen_core_task_h) -> c_int, 0, name, use_thread, task)
    }

    pub unsafe fn tizen_core_task_destroy(task: tizen_core_task_h) -> c_int {
        dlsym_call!(LIB_TIZEN_CORE, b"tizen_core_task_destroy\0", unsafe extern "C" fn(tizen_core_task_h) -> c_int, 0, task)
    }

    pub unsafe fn tizen_core_task_run(task: tizen_core_task_h) -> c_int {
        dlsym_call!(LIB_TIZEN_CORE, b"tizen_core_task_run\0", unsafe extern "C" fn(tizen_core_task_h) -> c_int, 0, task)
    }
...
pub mod vconf {
    ...
    pub unsafe fn vconf_get_str(key: *const c_char) -> *mut c_char {
        dlsym_call!(LIB_VCONF, b"vconf_get_str\0", unsafe extern "C" fn(*const c_char) -> *mut c_char, std::ptr::null_mut(), key)
    }

    pub unsafe fn vconf_get_int(key: *const c_char, val: *mut c_int) -> c_int {
        dlsym_call!(LIB_VCONF, b"vconf_get_int\0", unsafe extern "C" fn(*const c_char, *mut c_int) -> c_int, { *val = 0; 0 }, key, val)
    }

    pub unsafe fn vconf_set_str(key: *const c_char, val: *const c_char) -> c_int {
        dlsym_call!(LIB_VCONF, b"vconf_set_str\0", unsafe extern "C" fn(*const c_char, *const c_char) -> c_int, 0, key, val)
    }

    pub unsafe fn vconf_set_int(key: *const c_char, val: c_int) -> c_int {
        dlsym_call!(LIB_VCONF, b"vconf_set_int\0", unsafe extern "C" fn(*const c_char, c_int) -> c_int, 0, key, val)
    }
}
...
pub mod pkgmgr {
    ...
    pub unsafe fn pkgmgr_client_new(client_type: c_int) -> *mut pkgmgr_client {
        dlsym_call!(LIB_PKGMGR, b"pkgmgr_client_new\0", unsafe extern "C" fn(c_int) -> *mut pkgmgr_client, std::ptr::null_mut(), client_type)
    }
...
pub mod soup {
    ...
    pub type SoupServerCallback = unsafe extern "C" fn(*mut SoupServer, *mut SoupMessage, *const c_char, *mut c_void, *mut c_void, gpointer);

    pub const SOUP_MEMORY_COPY: c_int = 1;

    pub unsafe fn g_object_unref(object: gpointer) {
        dlsym_call!(LIB_SOUP, b"g_object_unref\0", unsafe extern "C" fn(gpointer), (), object)
    }
    ...
    pub unsafe fn g_main_loop_new(context: *mut GMainContext, is_running: gboolean) -> *mut GMainLoop {
        dlsym_call!(LIB_SOUP, b"g_main_loop_new\0", unsafe extern "C" fn(*mut GMainContext, gboolean) -> *mut GMainLoop, std::ptr::null_mut(), context, is_running)
    }
    ...
    pub unsafe fn soup_server_get_type() -> GType {
        dlsym_call!(LIB_SOUP, b"soup_server_get_type\0", unsafe extern "C" fn() -> GType, 0)
    }

    pub unsafe fn soup_server_listen_all(server: *mut SoupServer, port: guint, options: c_int, error: *mut *mut GError) -> gboolean {
        dlsym_call!(LIB_SOUP, b"soup_server_listen_all\0", unsafe extern "C" fn(*mut SoupServer, guint, c_int, *mut *mut GError) -> gboolean, 1, server, port, options, error)
    }

    pub unsafe fn soup_server_add_handler(server: *mut SoupServer, path: *const c_char, callback: SoupServerCallback, user_data: gpointer, destroy: Option<unsafe extern "C" fn(gpointer)>) {
        dlsym_call!(LIB_SOUP, b"soup_server_add_handler\0", unsafe extern "C" fn(*mut SoupServer, *const c_char, SoupServerCallback, gpointer, Option<unsafe extern "C" fn(gpointer)>), (), server, path, callback, user_data, destroy)
    }

    pub unsafe fn soup_server_disconnect(server: *mut SoupServer) {
        dlsym_call!(LIB_SOUP, b"soup_server_disconnect\0", unsafe extern "C" fn(*mut SoupServer), (), server)
    }

    pub unsafe fn soup_message_set_status(msg: *mut SoupMessage, status_code: guint) {
        dlsym_call!(LIB_SOUP, b"soup_message_set_status\0", unsafe extern "C" fn(*mut SoupMessage, guint), (), msg, status_code)
    }

    pub unsafe fn soup_message_set_response(msg: *mut SoupMessage, content_type: *const c_char, resp_use: c_int, resp_body: *const c_char, resp_length: gsize) {
        dlsym_call!(LIB_SOUP, b"soup_message_set_response\0", unsafe extern "C" fn(*mut SoupMessage, *const c_char, c_int, *const c_char, gsize), (), msg, content_type, resp_use, resp_body, resp_length)
    }

    pub unsafe fn soup_message_headers_append(hdrs: *mut SoupMessageHeaders, name: *const c_char, value: *const c_char) {
        dlsym_call!(LIB_SOUP, b"soup_message_headers_append\0", unsafe extern "C" fn(*mut SoupMessageHeaders, *const c_char, *const c_char), (), hdrs, name, value)
    }
}
...
pub mod app_event {
    ...
    pub unsafe fn event_add_event_handler(event_name: *const c_char, callback: app_event_cb, user_data: *mut c_void, handler: *mut event_handler_h) -> c_int {
        dlsym_call!(LIB_APP_EVENT, b"event_add_event_handler\0", unsafe extern "C" fn(*const c_char, app_event_cb, *mut c_void, *mut event_handler_h) -> c_int, 0, event_name, callback, user_data, handler)
    }

    pub unsafe fn event_remove_event_handler(handler: event_handler_h) -> c_int {
        dlsym_call!(LIB_APP_EVENT, b"event_remove_event_handler\0", unsafe extern "C" fn(event_handler_h) -> c_int, 0, handler)
    }
}
...
pub mod app_control {
    ...
    pub type app_control_reply_cb = unsafe extern "C" fn(app_control_h, app_control_h, c_int, *mut c_void);

    pub unsafe fn app_control_create(app_control: *mut app_control_h) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_create\0", unsafe extern "C" fn(*mut app_control_h) -> c_int, 0, app_control)
    }

    pub unsafe fn app_control_destroy(app_control: app_control_h) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_destroy\0", unsafe extern "C" fn(app_control_h) -> c_int, 0, app_control)
    }

    pub unsafe fn app_control_set_operation(app_control: app_control_h, operation: *const c_char) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_set_operation\0", unsafe extern "C" fn(app_control_h, *const c_char) -> c_int, 0, app_control, operation)
    }

    pub unsafe fn app_control_set_app_id(app_control: app_control_h, app_id: *const c_char) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_set_app_id\0", unsafe extern "C" fn(app_control_h, *const c_char) -> c_int, 0, app_control, app_id)
    }

    pub unsafe fn app_control_set_uri(app_control: app_control_h, uri: *const c_char) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_set_uri\0", unsafe extern "C" fn(app_control_h, *const c_char) -> c_int, 0, app_control, uri)
    }

    pub unsafe fn app_control_add_extra_data(app_control: app_control_h, key: *const c_char, value: *const c_char) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_add_extra_data\0", unsafe extern "C" fn(app_control_h, *const c_char, *const c_char) -> c_int, 0, app_control, key, value)
    }

    pub unsafe fn app_control_get_extra_data(app_control: app_control_h, key: *const c_char, value: *mut *mut c_char) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_get_extra_data\0", unsafe extern "C" fn(app_control_h, *const c_char, *mut *mut c_char) -> c_int, 0, app_control, key, value)
    }

    pub unsafe fn app_control_send_launch_request(app_control: app_control_h, callback: Option<app_control_reply_cb>, user_data: *mut c_void) -> c_int {
        dlsym_call!(LIB_APP_CONTROL, b"app_control_send_launch_request\0", unsafe extern "C" fn(app_control_h, Option<app_control_reply_cb>, *mut c_void) -> c_int, 0, app_control, callback, user_data)
    }
}
...
pub mod system_info {
    ...
    pub unsafe fn system_info_get_platform_string(key: *const c_char, value: *mut *mut c_char) -> c_int {
        dlsym_call!(LIB_SYSTEM_INFO, b"system_info_get_platform_string\0", unsafe extern "C" fn(*const c_char, *mut *mut c_char) -> c_int, 0, key, value)
    }

    pub unsafe fn system_info_get_platform_int(key: *const c_char, value: *mut c_int) -> c_int {
        dlsym_call!(LIB_SYSTEM_INFO, b"system_info_get_platform_int\0", unsafe extern "C" fn(*const c_char, *mut c_int) -> c_int, 0, key, value)
    }

    pub unsafe fn system_info_get_platform_bool(key: *const c_char, value: *mut c_int) -> c_int {
        dlsym_call!(LIB_SYSTEM_INFO, b"system_info_get_platform_bool\0", unsafe extern "C" fn(*const c_char, *mut c_int) -> c_int, 0, key, value)
    }
}
...
pub mod alarm {
    ...
    pub unsafe fn alarm_schedule_after_delay(app_control: *mut c_void, delay: c_int, period: c_int, alarm_id: *mut alarm_id_t) -> c_int {
        dlsym_call!(LIB_ALARM, b"alarm_schedule_after_delay\0", unsafe extern "C" fn(*mut c_void, c_int, c_int, *mut alarm_id_t) -> c_int, 0, app_control, delay, period, alarm_id)
    }

    pub unsafe fn alarm_cancel(alarm_id: alarm_id_t) -> c_int {
        dlsym_call!(LIB_ALARM, b"alarm_cancel\0", unsafe extern "C" fn(alarm_id_t) -> c_int, 0, alarm_id)
    }

    pub unsafe fn alarm_cancel_all() -> c_int {
        dlsym_call!(LIB_ALARM, b"alarm_cancel_all\0", unsafe extern "C" fn() -> c_int, 0)
    }
}
...
pub mod bundle {
    ...
    pub unsafe fn bundle_create() -> *mut bundle {
        dlsym_call!(LIB_BUNDLE, b"bundle_create\0", unsafe extern "C" fn() -> *mut bundle, std::ptr::null_mut())
    }

    pub unsafe fn bundle_free(b: *mut bundle) -> c_int {
        dlsym_call!(LIB_BUNDLE, b"bundle_free\0", unsafe extern "C" fn(*mut bundle) -> c_int, 0, b)
    }

    pub unsafe fn bundle_add_str(b: *mut bundle, key: *const c_char, str_val: *const c_char) -> c_int {
