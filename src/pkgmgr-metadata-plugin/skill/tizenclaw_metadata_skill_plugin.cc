...
constexpr const char kMetadataTizenclawSkill[] =
...
extern "C" EXPORT int PKGMGR_MDPARSER_PLUGIN_INSTALL(const char* pkgid,
                                                     const char* appid,
                                                     GList* metadata) {
  LOG(INFO) << "Skill plugin install: package=" << pkgid;
  GList* iter = metadata;
  while (iter != nullptr) {
    __metadata_t* md = static_cast<__metadata_t*>(iter->data);
    if (!strcmp(md->key, kMetadataTizenclawSkill)) {
      ...
      LOG(INFO) << "Package(" << pkgid
                << ") has valid platform signature for TizenClaw skill: "
                << (md->value ? md->value : "(empty)");
      break;
    }
...
extern "C" EXPORT int PKGMGR_MDPARSER_PLUGIN_UPGRADE(const char* pkgid,
                                                     const char* appid,
                                                     GList* metadata) {
  LOG(INFO) << "Skill plugin upgrade: package=" << pkgid;
  GList* iter = metadata;
  while (iter != nullptr) {
    __metadata_t* md = static_cast<__metadata_t*>(iter->data);
    if (!strcmp(md->key, kMetadataTizenclawSkill)) {
