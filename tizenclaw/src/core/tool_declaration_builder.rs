...
impl ToolDeclarationBuilder {
            ...
            if path.is_dir() {
                ...
                if skill_md.exists() {
                    if let Ok(content) = std::fs::read_to_string(&skill_md) {
                        if let Some(decl) = Self::parse_skill_md(&content, &path) {
                            tools.push(decl);
                        }
                    }
                }
    ...
    fn parse_skill_md(content: &str, skill_dir: &std::path::Path) -> Option<LlmToolDecl> {
        let mut name = String::new();
        let mut description = String::new();
        let mut params = json!({"type": "object", "properties": {}, "required": []});
        ...
        if name.is_empty() {
            name = skill_dir.file_name()?.to_str()?.to_string();
        }

        Some(LlmToolDecl { name, description, parameters: params })
    }
